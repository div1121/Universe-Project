...
void sendDataToOpenGL()
{
	...
	//alienpeople
	obj[0] = loadOBJ("resource/alienpeople.obj");
	...
	//alienpeople texture
	textureall[0].setupTexture("resource/texture/alienTexture.bmp");

	//alienvehicle
	obj[1] = loadOBJ("resource/alienvehicle.obj");
	...
	//chicken texture
	textureall[2].setupTexture("resource/texture/chickenTexture.bmp");
	...
	//rock texture
	textureall[4].setupTexture("resource/texture/rockTexture.bmp");
	...
	//pineapple texture
	textureall[7].setupTexture("resource/texture/10200_Pineapple.jpg");
	...
	//apple texture
	textureall[8].setupTexture("resource/texture/istockphoto-512401658-612x612.jpg");
	...
	//banana texture
	textureall[9].setupTexture("resource/texture/1200px-ICS_Quebec.svg.png");
...
void Renderthescene(Shader &tempshader) {
	...
	//alien2
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(5.0f, 3.0f, -100.0f));
	model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
	tempshader.setMat4("model", model);
	textureall[0].bind(0);
	glBindVertexArray(vaoID[0]);
	...
	tempshader.setMat4("model", model);
	textureall[alienvehiculeindex[0]].bind(0);
	glBindVertexArray(vaoID[1]);
	glDrawElements(GL_TRIANGLES, obj[1].indices.size(), GL_UNSIGNED_INT, 0);
	textureall[alienvehiculeindex[0]].unbind();
	...
	model = glm::scale(model, glm::vec3(1.2f, 1.2f, 1.2f));
	tempshader.setMat4("model", model);
	textureall[alienvehiculeindex[1]].bind(0);
	glBindVertexArray(vaoID[1]);
	glDrawElements(GL_TRIANGLES, obj[1].indices.size(), GL_UNSIGNED_INT, 0);
	textureall[alienvehiculeindex[1]].unbind();
	...
	model = glm::scale(model, glm::vec3(1.2f, 1.2f, 1.2f));
	tempshader.setMat4("model", model);
	textureall[alienvehiculeindex[2]].bind(0);
	glBindVertexArray(vaoID[1]);
	glDrawElements(GL_TRIANGLES, obj[1].indices.size(), GL_UNSIGNED_INT, 0);
	textureall[alienvehiculeindex[2]].unbind();
